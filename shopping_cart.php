...
$products_query = $conn->prepare("SELECT product_id, name, price FROM products ORDER BY product_id ASC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        case 'add':
            $product = getProductDetails($_POST['product_id']);
            if ($product && isset($product['name'], $product['price'])) {
                addItemToCart($_POST['product_id'], $product['name'], $product['price'], $_POST['quantity']);
            } else {
